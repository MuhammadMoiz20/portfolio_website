...
export interface ServicesSectionProps {
  ...
  services: Service[];
  /**
   * Visual layout style (default: 'grid')
   */
  layout?: 'grid' | 'list' | 'carousel' | 'featured';
  /**
   * Whether to show a "View All Services" link (default: false)
   */
  ...
  viewAllUrl?: string;
  /**
   * Whether to enable category filtering (default: false)
   */
    ...
    <div className="mt-10 space-y-6">
      {items.map((service, index) => (
        <motion.div
          ...
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.3, delay: index * 0.05 }}
          className={`flex rounded-lg border border-gray-200 p-6 transition-shadow hover:shadow-md dark:border-gray-700 dark:bg-gray-800 dark:hover:shadow-gray-800/30 ${
            service.featured
              ? 'bg-primary-50 dark:bg-primary-900/20'
              : 'bg-white dark:bg-gray-800'
          }`}
